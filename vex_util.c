...
struct stream_def *make_stream_def(struct llist *bitstream,
				   struct dvalue *pin,
				   struct dvalue *stream)
{
	      ...
	      int *name, char **value, char **units)
{
  ...
  *value=NULL;
  ...
  case 1:
  case 2:
    ierr=get_bitstream_list_field(stream_def->bitstream,n,link,name,
			       value, units);
    if(ierr==-1)
      return -1;
    else if (ierr!=0) {
      fprintf(stderr,"unknown error 1 in get_stream_def_field %d\n",ierr);
      exit(1);
    }
    break;
  case 3:
      *value=stream_def->pin->value;
      *units=stream_def->pin->units;
      *name=0;
      break;
  case 4:
      *value=stream_def->stream->value;
      *units=stream_def->stream->units;
      *name=0;
      break;
  default:
    return -1;
