...
%token <ival>   T_STREAM_DEF T_FORMAT_DEF T_THREAD_DEF T_CHANNEL_DEF
...
%token <ival>   B_TAPELOG_OBS B_BITSTREAMS B_THREADS
...
%type  <thptr>  thread_def
%type  <chptr>  channel_def

%type  <exptr>  external_ref
%type  <llptr>  literal
%type  <llptr>  unit_list name_list value_list
%type  <llptr>  unit_more
%type  <dvptr>  unit_value value optional_value optional_unit_value
...
bitstreams_lowl:   stream_def	        {$$=make_lowl(T_STREAM_DEF,$1);}
		   ...
		   | T_COMMENT          {$$=make_lowl(T_COMMENT,$1);}
                   | T_COMMENT_TRAILING {$$=make_lowl(T_COMMENT_TRAILING,$1);}
;
stream_def:	   T_STREAM_DEF '=' bit_stream_list ':' value ':' value ';'
